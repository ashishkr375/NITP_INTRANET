...
const FacultyHeader = ({ name, image, research, designation, email, linkedin, orcid, personal_webpage, google_scholar, vidwan, scopus }) => {
  return (
    <div className='h-fit md:h-5/6 md:rounded-lg bg-[#941B0C] p-2 md:flex md:mx-2 shadow-2xl text-xs md:text-base'>

      {/* image of professor */}

      <div className='w-full flex justify-center items-center md:w-fit'>{image ? <Image alt='blank' src={image} className='rounded-full h-32 w-32 md:h-44 md:w-52 md:mx-8 md:hover:h-48 md:hover:w-60' width={250} height={250} loading='lazy' /> : <CgProfile className='rounded-full h-20 w-20 md:h-44 md:w-52 md:mx-8'></CgProfile>}</div>
      ...
      <div className='w-full flex flex-col  mt-5 border hover:shadow-3xl hover:bg-[#ac4b3c] shadow-xl rounded-md p-2 md:mt-2 md:mx-4' style={{ boxShadow: "2px 2px 2px 1px black" }}>
        ...
        <div className='flex flex-col lg:flex-row lg:justify-between'>
          ...
          <div className='flex flex-col lg:w-[60%] gap-2 text-xs md:text-lg'>
          ...
          {/* <div className='flex flex-col lg:w-[30%]  mt-2 ml-3'>
            ...
            {personal_webpage?<Link href={`${personal_webpage}`} className='  bg-red-700 border p-2 text-xs w-fit rounded-md hover:shadow-2xl  m-3 shadow-xl font-semibold text-white'>Personal Webpage</Link>:<></>}
          </div> */}
        </div>
      </div>
    </div>
